#!/usr/bin/env python
"""
Phase vs X Position Visualization

Shows phase angle vs X position with cycle overlay.
"""
...
def render_video(
    ...
    with sio.VideoWriter(output_path, fps=fps, crf=crf) as writer:
        for frame_idx in tqdm(range(n_frames)):
            ...
            for phase in [0, 90, 180, 270, 360]:
                py = phase_to_plot_y(phase)
            ...
            canvas.drawLine(plot_x_start, phase_to_plot_y(180), plot_x_end, phase_to_plot_y(180), paint_center)
            ...
            for cycle in range(min_visible_cycle, current_cycle + 1):
                ...
                for i in range(len(cycle_frames) - 1):
                    ...
                    if np.isnan(centroids_cm_x[f1]) or abs(phase_deg[f2] - phase_deg[f1]) > 180:
                        continue
                    ...
                    canvas.drawLine(cm_to_plot_x(centroids_cm_x[f1]), phase_to_plot_y(phase_deg[f1]),
                                   cm_to_plot_x(centroids_cm_x[f2]), phase_to_plot_y(phase_deg[f2]), paint)

            if not np.isnan(centroids_cm_x[frame_idx]):
                r, g, b = get_phase_color(phase_deg[frame_idx])
                canvas.drawCircle(cm_to_plot_x(centroids_cm_x[frame_idx]), phase_to_plot_y(phase_deg[frame_idx]), 6 * s,
            ...
            canvas.drawString("X vs Phase", video_width_scaled + 10 * s, 20 * s, font, paint_text)
            ...
            for phase in [0, 180, 360]:
                canvas.drawString(f"{phase}\u00b0", plot_x_start - 25 * s, phase_to_plot_y(phase) + 4 * s, font_small, paint_text)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        output_path = str(script_dir / "phase_vs_x.mp4")
        scale = 1.0
